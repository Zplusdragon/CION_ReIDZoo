...
class TransReID_ViT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, stride_size=16, in_chans=3, num_classes=0, embed_dim=768,
        ...
        self.patch_embed = PatchEmbed_VOLO(img_size=img_size, stem_conv=True, stem_stride=2
                                           , patch_size=patch_size, in_chans=in_chans, hidden_dim=64,
                                           embed_dim=embed_dim)
...
def vit_tiny(image_size=(256, 128), patch_size=16, stride_size=16, drop_rate=0., attn_drop_rate=0.,
                   drop_path_rate=0.0, **kwargs):
    #  kwargs.setdefault('qk_scale', 768 ** -0.5)
    print('ICS Tiny')
    model = TransReID_ViT(img_size=image_size, patch_size=patch_size, stride_size=stride_size, embed_dim=192, depth=12,
                     num_heads=3, mlp_ratio=4., qkv_bias=True, drop_path_rate=drop_path_rate, drop_rate=drop_rate,
                     attn_drop_rate=attn_drop_rate, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model


def vit_small(image_size=(256, 128), patch_size=16, stride_size=16, drop_rate=0., attn_drop_rate=0.,
                   drop_path_rate=0.0, **kwargs):
    #  kwargs.setdefault('qk_scale', 768 ** -0.5)
    print('ICS Small')
    model = TransReID_ViT(img_size=image_size, patch_size=patch_size, stride_size=stride_size, embed_dim=384, depth=12,
                     num_heads=6, mlp_ratio=4., qkv_bias=True, drop_path_rate=drop_path_rate, drop_rate=drop_rate,
                     attn_drop_rate=attn_drop_rate, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model


def vit_base(image_size=(256, 128), patch_size=16, stride_size=16, drop_rate=0., attn_drop_rate=0.,
                  drop_path_rate=0., **kwargs):
    print('ICS Base')
    print(drop_rate)
    print(image_size)
    model = TransReID_ViT(img_size=image_size, patch_size=patch_size, stride_size=stride_size, embed_dim=768, depth=12,
                     num_heads=12, mlp_ratio=4., qkv_bias=True, drop_path_rate=drop_path_rate, drop_rate=drop_rate,
                     attn_drop_rate=attn_drop_rate, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model
