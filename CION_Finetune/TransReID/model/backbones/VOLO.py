...
class VOLO(nn.Module):
    ...
    """
    def __init__(self, layers, img_size=[256,128], in_chans=3, num_classes=1000, patch_size=8,
                 ...
                 post_layers=None, return_mean=False,pooling_scale=2, out_kernel=3, out_stride=2, out_padding=1):
        ...
        self.pos_embed = nn.Parameter(
            torch.zeros(1, img_size[0] // patch_size // pooling_scale,
    ...
    def interpolate_pos_encoding(self, x, h, w):
        ...
        N = self.pos_embed.shape[1]*self.pos_embed.shape[2]
        if npatch == N:
            return self.pos_embed
        OH = self.img_size[0] // self.patch_size // self.pooling_scale
        OW = self.img_size[1] // self.patch_size // self.pooling_scale
        patch_pos_embed = self.pos_embed
        dim = x.shape[-1]
        # we add a small number to avoid floating point error in the interpolation
        # see discussion at https://github.com/facebookresearch/dino/issues/8
        w0 = w // self.patch_size // self.pooling_scale
        h0 = h // self.patch_size // self.pooling_scale
        w0, h0 = w0 + 0.1, h0 + 0.1
    ...
    def forward(self, x):
        ...
        x,w,h = self.forward_embeddings(x)

        # step2: tokens learning in the two stages
        x = self.forward_tokens(x,w,h)
