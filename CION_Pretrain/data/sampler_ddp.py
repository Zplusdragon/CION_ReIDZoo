...
class RandomIdentitySampler_DDP(Sampler):
    ...
    def __init__(self, data_source, batch_size, num_instances):
        ...
        for index, item in enumerate(self.data_source.dataset):
            pid = item["id"]
    ...
    def __iter__(self):
        ...
        if self.rank == 0 :
            ...
            idxs_size = len(final_idxs)
        else:
            idxs_size = 0

        idxs_size_tensor = torch.tensor([idxs_size]).cuda()
        dist.broadcast(tensor=idxs_size_tensor,src=0)
        idxs_size = idxs_size_tensor.item()

        if self.rank!=0:
            final_idxs_tensor = torch.zeros(idxs_size,dtype=torch.int64).cuda()
        else:
            final_idxs_tensor = torch.tensor(final_idxs,dtype=torch.int64).cuda()

        dist.broadcast(final_idxs_tensor,src=0)

        #dist.barrier()
        final_idxs = final_idxs_tensor.tolist()
        #length = int(math.ceil(len(final_idxs) * 1.0 / self.world_size))

        length = len(final_idxs)//self.world_size
        length = (length//self.num_instances)*self.num_instances
                ...
                if pid not in batch_idxs_dict:
                    ...
                    if len(idxs) < self.num_instances:
                        ...
                        for i in range(self.num_instances):
                            selected_idxs.extend(random.sample(idxs,1))
                        idxs = selected_idxs

                    if len(idxs) >= self.num_instances and len(idxs)//self.num_instances!=0:
                        supplement_num = len(idxs)-(len(idxs)//self.num_instances)*self.num_instances
                        idxs.extend(random.sample(idxs,supplement_num))
