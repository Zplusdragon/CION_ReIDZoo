...
def vit_tiny_p16(img_size=(256, 128), patch_size=16, drop_rate=0., attn_drop_rate=0.,
                   drop_path_rate=0.0, **kwargs):
    print('ICS ViT-T/16')
    model = TransReID_ViT(img_size=img_size, patch_size=patch_size, embed_dim=192, depth=12,
...
def vit_small_p16(img_size=(256, 128), patch_size=16, drop_rate=0., attn_drop_rate=0.,
                   drop_path_rate=0.0, **kwargs):
    print('ICS ViT-S/16')
    model = TransReID_ViT(img_size=img_size, patch_size=patch_size,  embed_dim=384, depth=12,
                     num_heads=6, mlp_ratio=4., qkv_bias=True, drop_path_rate=drop_path_rate, drop_rate=drop_rate,
                     attn_drop_rate=attn_drop_rate, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model


def vit_base_p16(img_size=(256, 128), patch_size=16, drop_rate=0., attn_drop_rate=0.,
                  drop_path_rate=0.0, **kwargs):
    print('ICS ViT-B/16')
    model = TransReID_ViT(img_size=img_size, patch_size=patch_size,  embed_dim=768, depth=12,
